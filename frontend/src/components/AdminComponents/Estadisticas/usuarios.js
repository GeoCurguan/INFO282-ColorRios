...
const Usuarios = ({ dataUsers, dataPalettes, dataPalettesLR }) => {
  const usersInLosRios = dataUsers ? dataUsers.filter((user) => user.region === "Los Ríos") : [];
  ...
  return (
    <Grid numItems={1} numItemsSm={2} className="gap-3">
      <Card decoration="top" decorationColor="fuchsia">
        <Title className="dark:text-dark-tremor-content-emphasis">Usuarios Registrados de la Región de Los Ríos</Title>
        <Flex justifyContent="start" alignItems="baseline" className="truncate space-x-3">
          <Metric>{userCount}</Metric>
          <Text className="truncate">de un total de {userTotal} usuarios</Text>
        </Flex>
      </Card>
      <Card decoration="top" decorationColor="fuchsia">
        ...
        <Flex justifyContent="start" alignItems="baseline" className="truncate space-x-3">
          ...
          <Text className="truncate">de un total de {paletteTotal} paletas</Text>
        </Flex>
      </Card>
    </Grid>
  );
